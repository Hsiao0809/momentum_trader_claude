...
- Position recovery: normal ticks request only 8 recent 15-minute candles, while delayed positions expand dynamically up to 300 candles (about 75 hours). Before applying any candle, the Worker verifies that history continues from the position's `lastTime`; an unbridgeable gap leaves `lastTime` unchanged, reports `position_history_gap`, and pauses new entries until complete history is available.
