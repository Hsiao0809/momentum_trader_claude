...
function evaluateSignal(symbol, instId, rows, quoteVolume, scannedAt, riskOff, cfg) {
  ...
  if (aboveEma && emaSlope >= 0) {
    ...
    // smbChaseHot：突破成立時放行 smb 追噴出（跳過 1h≥6 通用拒絕與後面的確認），噴出中一成立就進（見 DEFAULT_CFG 註解）
    const smbBreakout = (momentum4h >= 15 && position24h >= 0.80) || (momentum24h >= 20 && distancePrevHigh >= -2);
    const smbHotEntry = cfg.smbChaseHot && smbBreakout;
    if (momentum1h >= 6 && !smbHotEntry) return null;
    ...
    // strong_momentum_breakout 進場確認（見 DEFAULT_CFG smbConfirmBars 註解）；smbHotEntry(追噴出) 時跳過確認、一成立就進
    if (key === 'strong_momentum_breakout' && !smbHotEntry) {
