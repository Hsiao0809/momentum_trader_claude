...
assert.match(source, /scanBatchSize: 4/);
assert.match(source, /xyzScanLimit: 4/);
assert.match(source, /scanStaleMs: 10 \* 60 \* 1000/);
assert.match(source, /const willScan = options\.forceScan \|\| hasActiveScanPlan\(state\) \|\| scanStale/);
assert.match(source, /function createScanPlan\(state\)/);
